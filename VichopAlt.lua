...
local whitelistedPlayerIDs = {
    80299238,  -- Example whitelisted player ID
}

-- Define webhook URL and role IDs (these would typically be passed from the loader script)
local url = "https://discord.com/api/webhooks/1253108111351349278/eeYN9I-015aDEo0T9L7ch8aLrZSMCGNKT4M3pRQEn34eLtqM1E9FVN13uIw35dna7S3k"
...
local function CheckWhitelistAndProceed(player)
    ...
    if IsPlayerWhitelisted(player) then
        ...
        if viciousBee then
            ...
            SendMessageEMBED(url, embed, true)  -- Uses role IDs in 'roleIDs' table
            wait(5)
...
-- Listen for player joining to check whitelist
...
-- Initial check for the local player
